...
const NewChatList: React.FC<MatchesListProps> = ({ handleSelectUser, onClose}) => {
  ...
  /**
   * It fetches the user's matches from an API and allows the current user to initiate one-on-one chat conversations with those matches,
   * provided no previous chat that is NOT a study group exists between the users. 
   */
  

  // useEffect hook to fetch matches when the component mounts
  useEffect(() => {
    const fetchMatches = async () => {
        try {
          ...
          // Filter out study group matches
  ...
  // Handle message button click - initiate a new chat if no chat exists
  const handleMessage  = async (userId: number, currentId: number | null) => {
      ...
      try {
          ...
          // Make an API request to create the new chat
          const response = await axios.post(`${REACT_APP_API_URL}${endpoint}`, payload);
      
      
          if (response.status === 200 || response.status === 201) {
            // If chat was created successfully, update parent component and navigate to the chat
            ...
            window.location.reload(); // Reload page to reflect the new chat
